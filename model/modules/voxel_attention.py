...
import math
from model.modules.voxel_attention import SelfAttentionBlock
from ....utils
...
class PositionalEncoding(nn.Module):
    ...
    def __init__(self, d_model, height, width, depth=2):
        ...
        self.depth = depth
        # Not a parameter
        self.register_buffer('pos_table', self.positional_encoding())
    ...
    def positional_encoding(self):
        ...
        div_term_depth = torch.exp(torch.arange(
            0., d_model-2, 2) * -(math.log(10000.0) / d_model-2))
        ...
        pe[0:d_model:2, :, :, :] = torch.sin(pos_w * div_term).transpose(
            0, 1).unsqueeze(1).unsqueeze(-1).repeat(1, self.height, 1, self.depth)
        pe[1:d_model:2, :, :, :] = torch.cos(pos_w * div_term).transpose(
            0, 1).unsqueeze(1).unsqueeze(-1).repeat(1, self.height, 1, self.depth)
        pe[d_model:2 * d_model:2, :, :, :] = torch.sin(pos_h * div_term).transpose(
            0, 1).unsqueeze(2).unsqueeze(-1).repeat(1, 1, self.width, self.depth)
        pe[d_model + 1:2 * d_model:2, :, :, :] = torch.cos(pos_h * div_term).transpose(
            0, 1).unsqueeze(2).unsqueeze(-1).repeat(1, 1, self.width, self.depth)
        pe[2 * d_model::2, :, :, :] = torch.sin(pos_d * div_term_depth).transpose(
            0, 1).unsqueeze(1).unsqueeze(2).repeat(1, self.height, self.width, 1)
        pe[2 * d_model + 1::2, :, :, :] = torch.cos(pos_d * div_term_depth).transpose(
            0, 1).unsqueeze(1).unsqueeze(2).repeat(1, self.height, self.width, 1)
        return pe
...
class VoxelContext3D_fsa(nn.Module):
    def __init__(self, model_cfg, grid_size, voxel_size, point_cloud_range, dropout=0.1):
        ...
        self.self_attn1 = SelfAttentionBlock(
            inplanes=self.model_cfg.IN_DIM, planes=self.model_cfg.IN_DIM)
        self.self_attn2 = SelfAttentionBlock(
            inplanes=self.model_cfg.IN_DIM, planes=self.model_cfg.IN_DIM)
    ...
    def add_context_to_voxels(self, voxel_features, coords, nx, ny, nz):
        ...
        for batch_idx in range(batch_size):
            ...
            voxel_pillars = voxel_pillars.permute(
                0, 2, 1).contiguous().squeeze(0)

            this_coords = coords[batch_mask, :]
            indices = this_coords[:, 1] + \
                this_coords[:, 2] * nx + this_coords[:, 3]
        ...
        voxel_pillar_features = voxel_pillar_features.view(
            batch_size, self.model_cfg.NUM_BEV_FEATURES * nz, ny, nx)
    ...
    def forward(self, batch_dict):
        ...
        voxel_pos_enc = self.dropout(
            self.position_enc(voxel_feats, cur_coords))
        voxel_pos_enc = self.layer_norm(voxel_pos_enc)

        # get context for every voxel
        voxel_context_features = self.add_context_to_voxels(
            voxel_pos_enc, cur_coords, nx, ny, nz)

        # generate down-sampled self-attention-features to concatenate with Conv
        voxel_context = [voxel_context_features,
                         F.interpolate(voxel_context_features,
